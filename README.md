...
**Preconditions:** Prior to running the body calibration, it needs to have access to the dual LiDAR alignment calibration and the floor calibration provided by running the following command. You should leave this running in a separate terminal.

> > > > > > > a393100 (Update to new link and joint names, publish floor plane instead of base footprint)
...
**Preconditions:** Prior to running body calibration, it’s especially important to stow the robot’s arm as described above and ensure that the wrist and end effector are within the footprint of the mobile base. _Ensure `ros_broadcast_calibration` is running in another terminal before doing this!_

```bash
ros_find_body_calibration
```
...
- **Result**: The transform `floor_plane_to_base_footprint_transform` is saved to `dual_lidar_calibration.yaml`. This transform mathematically maps points natively from the `base_footprint` frame into the `floor_plane` frame, correcting for default physical errors.
